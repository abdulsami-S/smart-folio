...
const FloatingHeading = ({ words, className, style, delay = 0, isDark = true }) => {
  ...
  const gradient = isDark
    ? 'linear-gradient(90deg, #ffd700, #f5a623, #e8751a, #f5a623, #ffd700)'
    : 'linear-gradient(90deg, #4a1942, #b5451b, #e8751a, #b5451b, #4a1942)';
  useEffect(() => {
  ...
  return (
    <span ref={ref} className={className} style={{ ...style, display: 'inline-flex', flexWrap: 'wrap', gap: '0.22em' }}>
      {words.map((word, i) => (
        <span key={i} className="fg-word" style={{ display: 'inline-block', willChange: 'transform', background: gradient, backgroundSize: '300% auto', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
          {word}
        </span>
      ))}
    </span>
...
/* ─── BUTTON — IDLE FLOAT + HOVER ────────────────────────────────────────────
 * Two styles:
 *   primary  — solid gradient fill, asymmetric corners, arrow icon
 *   outline  — ghost border, fills on hover, download icon
 * Idle float animation, GSAP hover effects. No magnetic cursor pull.
 * ──────────────────────────────────────────────────────────────────────────── */
const MagneticBtn = ({ children, className, style, href, target, rel, variant = 'primary' }) => {
  ...
  useEffect(() => {
    ...
    // Idle float — different phase per variant
    gsap.to(wrap, {
      y: variant === 'primary' ? -6 : -4,
      duration: 1.8 + Math.random() * 0.8,
      repeat: -1, yoyo: true, ease: 'sine.inOut',
      delay: variant === 'primary' ? 0 : 0.6,
    });

    return () => gsap.killTweensOf(wrap);
  }, [variant]);

  // GSAP hover — drives variant-specific visual states
  const handleEnter = () => {
    const wrap = wrapRef.current;
    if (!wrap) return;
    if (variant === 'primary') {
      gsap.to(wrap, { boxShadow: '0 12px 32px rgba(201,112,74,0.5)', scale: 1.04, duration: 0.3 });
    } else {
      gsap.to(wrap, { borderColor: 'rgba(201,112,74,1)', backgroundColor: 'rgba(201,112,74,0.08)', scale: 1.04, duration: 0.3 });
    }
  };
  const handleLeave = () => {
    const wrap = wrapRef.current;
    if (!wrap) return;
    if (variant === 'primary') {
      gsap.to(wrap, { boxShadow: '0 8px 24px rgba(201,112,74,0.3)', scale: 1, duration: 0.3 });
    } else {
      gsap.to(wrap, { borderColor: 'rgba(201,112,74,0.5)', backgroundColor: 'rgba(201,112,74,0)', scale: 1, duration: 0.3 });
    }
  };
  ...
  return (
    <a ref={wrapRef} href={href} target={target} rel={rel}
      ...
      style={{ display: 'inline-flex', alignItems: 'center', willChange: 'transform', position: 'relative', ...style }}
      className={className}
    >
      <span ref={innerRef} style={{ display: 'flex', alignItems: 'center', gap: 'inherit', willChange: 'transform', position: 'relative', zIndex: 1 }}>
        {children}
      </span>
    </a>
  );
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-20 items-center">
          ...
          <div ref={introRef} className="flex flex-col gap-8" style={{ opacity:0 }}>
            ...
            <div ref={buttonsRef} className="flex flex-wrap gap-5" style={{ opacity:0 }}>
              ...
              {/* Primary CTA — direct styles, no absolute layers */}
              <MagneticBtn
                href="#projects"
                variant="primary"
                style={{
                  padding:'14px 32px',
                  background:'linear-gradient(135deg, #c9704a, #9b3d1e)',
                  color:'#fff3e6',
                  ...
                  boxShadow:'0 8px 24px rgba(201,112,74,0.3)',
                  gap:'0.6rem',
                  textDecoration:'none',
                }}
              ...
              {/* Outline CTA — direct styles, no absolute layers */}
              <MagneticBtn
                ...
                style={{
                  ...
                  letterSpacing:'0.18em',
                  textTransform:'uppercase',
                  border:'1px solid rgba(201,112,74,0.5)',
                  gap:'0.6rem',
                  textDecoration:'none',
